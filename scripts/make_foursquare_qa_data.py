...
if __name__ == "__main__":
    ...
    input_dir_qa = get_path_from_config("input_directory", "QA")
    input_pqt_qa = input_dir_qa / 'parquet'
    schema_csv_qa = get_schema_csv(input_dir_qa / 'schema')
    output_fc_qa = get_path_from_config("output_feature_class", "QA")

    input_dir_sample = get_path_from_config("input_directory", "QA_SAMPLE")
    input_pqt_qa_sample = input_dir_sample / 'parquet'
    schema_csv_qa_sample = get_schema_csv(input_dir_sample / 'schema')
    output_fc_qa_sample = get_path_from_config("output_feature_class", "QA_SAMPLE")

    output_fgdb = output_fc_qa.parent
    ...
    fh = logging.FileHandler(str(output_dir / f"foursquare_qa_{timestamp_str}.log"))
    ...
    if not input_pqt_qa.exists():
        raise ValueError(f'Cannot locate the input parquet dataset {input_pqt_qa}')

    # convert AOI quality assurance to feature class
    logging.info('Starting Areas of Interest parquet data import.')
    parquet_to_feature_class(
        parquet_path=input_pqt_qa,
        geometry_type='COORDINATES',
        geometry_column=['longitude', 'latitude'],
        output_feature_class=output_fc_qa,
        schema_file=schema_csv_qa,
        spatial_reference=4326,
        build_spatial_index=True
    )

    logging.info('Starting sample parquet data import.')
    parquet_to_feature_class(
        parquet_path=input_pqt_qa_sample,
        output_feature_class=output_fc_qa_sample,
        geometry_type='COORDINATES',
        geometry_column=['longitude', 'latitude'],
        schema_file=schema_csv_qa_sample,
        spatial_reference=4326,
        build_spatial_index=True
    )
